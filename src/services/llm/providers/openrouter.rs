...
impl OpenRouterClient {
    ...
    pub async fn chat_stream(
        ...
        Err(ModelClientError::Unsupported("OpenRouter streaming not implemented yet".into()))
    }

    pub async fn embeddings(&self, _req: EmbeddingsRequest) -> Result<EmbeddingsResponse, ModelClientError> {
        Err(ModelClientError::Unsupported("OpenRouter embeddings not implemented yet".into()))
    }
}
